      ...
      >
          ...
          >
            {sortedRegions.map((region, index) => {
              const percent =
                totalOrders > 0
                  ? Math.round(
                      (Number(region.total_orders || 0) / totalOrders) * 100
                    )
                  : 0;
        ...
        >
          ...
          <div style={{ display: "grid", gap: "12px", marginTop: "16px" }}>
            ...
            >
              <strong>توصية:</strong>{" "}
             {selectedRegionPercent >= 30
  ? "هذه من أقوى المناطق حاليًا. ركّز عليها بحملة إعلانية أو عرض خاص لأنها تمثل نسبة عالية من الطلبات."
  : selectedRegionPercent >= 15
  ? "هذه المنطقة لديها طلب جيد. جرّب تحسين الشحن أو تقديم عرض محلي لزيادة المبيعات منها."
  : selectedRegion?.region === "غير محدد"
  ? "يوجد طلبات بدون منطقة واضحة. حسّن جمع بيانات العنوان حتى تصبح قراراتك الجغرافية أدق."
  : selectedRegion?.total_orders > 0
  ? "هذه المنطقة ما زالت ضعيفة نسبيًا. اختبر حملة صغيرة قبل زيادة الميزانية الإعلانية."
